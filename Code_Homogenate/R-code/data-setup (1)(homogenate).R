...
tissue="HOMOGENATE" # this will used in file names.
...
########## sample meta file
######################################
samples=fread("/data3/DownLoadedData/McDonald/ASD_Development_June2022/Homogenate/Run2/Shelby_Prelim_Analysis/Peptide_Normalization_ProteinRollup/Covariates/ASD_Dev_Clinic_allvariables.csv",
           header=T,data.table=F)
colnames(samples)
colnames(samples)=c("ID","AGEyr","PMIhr","RACE","SEX","DX","CONFIRMATION","COMORBIDITIES","CBD","PAIR","ID1","ID2")
samples$ID=paste0("S",samples$ID)
samples$PAIR=paste0("MP",substr(samples$PAIR,1,nchar(samples$PAIR)-1))
rownames(samples)=samples$ID2  # this is the ID information in the plex information file

# read the plex information for the samples
plex=fread("/data3/DownLoadedData/McDonald/ASD_Development_June2022/Homogenate/Run2/Shelby_Prelim_Analysis/Peptide_Normalization_ProteinRollup/Peptide_input_files/ASD_Dev_Clinic_PD.csv",
           header=T,data.table=F)
colnames(plex)=c("PD.ID","PLEX","TMT.PLEX","CHANNEL","RYAN.PLEX","RYAN.CHANNEL","ID2")
rownames(plex)=gsub("SD","",gsub("TL","",plex$ID2))
sum(rownames(plex) %in% samples$ID2)  # check to see if the IDs match between


#### create the meta file for the samples and pools
meta.samples=plex[,c("PD.ID","PLEX","CHANNEL")]
# add ROLE
meta.samples$ROLE="SAMPLE"
meta.samples$ROLE[substr(rownames(meta.samples),1,1) == "P"]="POOL"
...
meta.samples$SEX=factor(meta.samples$SEX, levels=c("Male","Female"))

meta.samples$DX[is.na(meta.samples$DX)]="POOL"
meta.samples$DX=factor(meta.samples$DX, levels=c("CTL","ASD","POOL"))

meta.samples$RACE=factor(meta.samples$RACE,levels=c("White","Black or African-American","Unknown"),labels=c("CAU","AFR","UNK"))
...
meta.samples$COMORBIDITIES=factor(meta.samples$COMORBIDITIES,levels=c("NONE","ADHD","CP","EPLSY","PW"))
...
# read the psm
psm=as.data.frame(read_excel("/data3/DownLoadedData/McDonald/ASD_Development_June2022/Homogenate/Run2/PD_export_originalfiles/PSM/ASD_Dev_Hom_Allplexes_Run2_PSM.xlsx")) # 365,221
head(psm)
...
rownames(meta.pep)=meta.pep$FEATURE
