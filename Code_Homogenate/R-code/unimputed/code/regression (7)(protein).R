...
effects=c("DXASD","AGEyr") # effects of interest as identified in the regression coefficients
RE=list()
...
##### this part does not work for this data  
# add the meta information to the proteins
#meta.pep=fread("WorkData/HOMOGENATE-meta-peptides.txt",header=T,data.table=F)
#meta.prt=meta.pep[!duplicated(meta.pep[,c("MASTER.PROTEIN","MASTER.GENE")]),]
#meta.prt=meta.prt[meta.prt$MASTER.PROTEIN %in% rownames(norm.prt),]
#
#rownames(meta.prt)=meta.prt$MASTER.PROTEIN
#for(effect in effects){
#  RE[[effect]][,c("MASTER.GENE")]=meta.prt[RE[[effect]]$protein,c("MASTER.GENE")]
#}
...
# save the results
if(!is.null(random.effect)){
  save(reREGR,RE.DX,RE.AGE,re.summary,meta.samples,samples,norm.prt,selected.model,file=paste0("WorkData/LMER-REGRESSION-",tissue,".RData"))
}else{
  save(reREGR,RE.DX,RE.AGE,re.summary,meta.samples,samples,norm.prt,selected.model,file=paste0("WorkData/LM-REGRESSION-",tissue,".RData"))
}
