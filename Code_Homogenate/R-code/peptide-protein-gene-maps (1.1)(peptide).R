...
load("WorkData/RAW-HOMOGENATE-PEPTIDE.RData")
...
proteins=unique(gsub(" ","",unlist(strsplit(meta.pep$protein[!is.na(meta.pep$protein)],";"))))  # 8660
...
reBM=getBM(c("uniprotswissprot","ensembl_gene_id","external_gene_name","chromosome_name"),filters="uniprotswissprot",
           values=proteins,mart=mart) # 9303
reBM=reBM[reBM$chromosome_name %in% c(1:23,"X","Y","MT"),]  # 8564

length(unique(reBM$uniprotswissprot))  # 8532
length(unique(reBM$ensembl_gene_id))    # 8559
length(unique(reBM$external_gene_name)) # 8558
