...
# this is sequential process where at each step the covariate that imrpoves the most features (peptides)
...
model.peptides=rownames(norm.pep) # list of peptides to model
...
repeat{
  # model to compare to
  system.time(re.null.BIC<-unlist(mclapply(model.peptides,determineBIC,Y=log2(norm.pep[model.peptides,samples]),XX=meta.samples[samples,],
  ...
  for(covariate in setdiff(possible.covariates,null.model.covariates)){
    ...
    re.alt.BIC<-unlist(mclapply(model.peptides,determineBIC,Y=log2(norm.pep[model.peptides,samples]),XX=meta.samples[samples,],
  ...
  rownames(delta.BIC)=model.peptides
  ...
  model.peptides=rownames(delta.BIC)[rowSums(delta.BIC) > 0]
  ...
  if(max(improved.BIC[1:(ncol(improved.BIC)-3)]) < nrow(norm.pep)*0.05 | nrow(re.model.selection) == length(possible.covariates)){break}
}

# some clean-up
selected.model=null.model.covariates
if(nrow(re.model.selection) < length(possible.covariates) | re.model.selection[nrow(re.model.selection),re.model.selection$selected[nrow(re.model.selection)]] < nrow(norm.pep)*0.05){
...
if(!is.null(random.effect)){
  save(meta.samples,meta.pep,norm.pep,selected.model,re.model.selection,file=paste0("WorkData/LMER-MODEL-SELECTION-",tissue,".RData"))
}else{
  save(meta.samples,meta.pep,norm.pep,selected.model,re.model.selection,file=paste0("WorkData/LM-MODEL-SELECTION-",tissue,".RData"))
}

### look for additional models including interactions with DX that might be of interest

re.null.BIC=unlist(mclapply(rownames(norm.pep),determineBIC,Y=log2(norm.pep[,samples]),XX=meta.samples[samples,],
                            covariates=selected.model,random.effect=random.effect,
                            mc.cores=64,mc.preschedule = T,mc.silent = T))
re.alt.BIC=unlist(mclapply(rownames(norm.pep),determineBIC,Y=log2(norm.pep[,samples]),XX=meta.samples[samples,],
                           covariates=c(selected.model,"AGEyr:DX"),random.effect=random.effect,
                           mc.cores=64,mc.preschedule = T,mc.silent = T))
sum(re.alt.BIC - re.null.BIC < delta.cut["AGEyr"])  # 4 < .05*nrow(norm.pep) = 128.9

#selected model + interaction of AGE:DX
#improved 207 features

#selected model + interaction of AGE:DX + AGE^2:DX
#improved 180 features
...
### save the results with the input
if(!is.null(random.effect)){
  save(meta.samples,meta.pep,norm.pep,selected.model,re.model.selection,samples,file=paste0("WorkData/LMER-MODEL-SELECTION-",tissue,".RData"))
}else{
  save(meta.samples,meta.pep,norm.pep,selected.model,re.model.selection,samples,file=paste0("WorkData/LM-MODEL-SELECTION-",tissue,".RData"))
}