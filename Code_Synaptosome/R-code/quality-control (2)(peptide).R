...
dim(raw.pep)  # 61,060 x 74

# remove peptides without intensity data
sum(rowSums(!is.na(raw.pep)) == 0)  # 5711
raw.pep=raw.pep[rowSums(!is.na(raw.pep)) > 0,] 
dim(raw.pep)  # 55,349 x 74
meta.pep=meta.pep[rownames(raw.pep),]  # 55,349


# remove peptides with missing master protein (ROLLUP) information   
sum(is.na(meta.pep$ROLLUP)) # - 121
meta.pep=meta.pep[!is.na(meta.pep$ROLLUP),]  
dim(meta.pep)                # 55,228


length(unique(unlist(strsplit(meta.pep$ROLLUP,";")))) # 7393
# select the corresponding peptide intensities
raw.pep=raw.pep[rownames(meta.pep),]  
dim(raw.pep)                     # 55,228 x 74
...
dim(raw.pep)  # 29,311 x 74
meta.pep=meta.pep[rownames(raw.pep),]
length(unique(unlist(strsplit(meta.pep$ROLLUP,";")))) # 5140
