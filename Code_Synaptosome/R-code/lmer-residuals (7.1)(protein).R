...
selected.model=c("DX","PLEX","AGEyr")
...
#system.time(res.prt <- mclapply(rownames(norm.prt),lmerResiduals,Y=log2(norm.prt[,samples]),XX=meta.samples[samples,],
 #                               covariates=selected.model,random.effect="PAIR",add.back=c("DXASD","AGEyr","DXASD:AGEyr"),mc.cores=1,mc.preschedule=T))
...
#residual_DX=res.prt
#residual=res.prt
#residual_DXandDXAGEint=res.prt
#residual_AGEandDXAGEint=res.prt
#residual_DXandAGEandDXAGEint=res.prt

# these are with no interaction in the model
...
save(residual,
    ...
    residual_AGE,
   residual_DXandAGE,
    residual_DXandDXAGEint,
    residual_AGEandDXAGEint,
    residual_DXandAGEandDXAGEint,
  meta.samples,
 norm.prt,
 file="Analysis_Code/Code_Synaptosome/WorkData/residuals.RData")
############
