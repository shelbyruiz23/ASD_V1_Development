...
re.alt.BIC=unlist(mclapply(rownames(norm.pep),determineBIC,Y=log2(norm.pep[,samples]),XX=meta.samples[samples,],
                           ...
                           mc.cores=64,mc.preschedule = T,mc.silent = T))
sum(re.alt.BIC - re.null.BIC < delta.cut["AGEyr"])  # 1055



re.null.BIC=unlist(mclapply(rownames(norm.pep),determineBIC,Y=log2(norm.pep[,samples]),XX=meta.samples[samples,],
                            covariates=c(selected.model, "AGEyr:DX"),
                            random.effect=random.effect,
                            mc.cores=64,mc.preschedule = T,mc.silent = T))
re.alt.BIC=unlist(mclapply(rownames(norm.pep),determineBIC,Y=log2(norm.pep[,samples]),XX=meta.samples[samples,],
                           covariates=c(selected.model,"AGEyr:DX","AGEyr2:DX"),
                           random.effect=random.effect,
                           mc.cores=64,mc.preschedule = T,mc.silent = T))
sum(re.alt.BIC - re.null.BIC < delta.cut["AGEyr2"])  # 180
