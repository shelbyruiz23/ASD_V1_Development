...
# initial run uses intercept and diagnosis
null.model.covariates="DX"    # these effects will be forced into the model
#null.model.covariates=c("DX","AGEyr","DX:AGEyr")    # these effects will be forced into the model
...
if(!is.null(random.effect)){
  save(meta.samples,meta.pep,norm.prt,samples,selected.model,re.model.selection,file=paste0("WorkData/LMER-MODEL-SELECTION-",tissue,".RData"))
}else{
  save(meta.samples,meta.pep,norm.prt,samples,selected.model,re.model.selection,file=paste0("WorkData/LM-MODEL-SELECTION-",tissue,".RData"))
}
...
re.alt.BIC=unlist(mclapply(rownames(norm.prt),determineBIC,Y=log2(norm.prt[,samples]),XX=meta.samples[samples,],
                           ...
                           mc.cores=1,mc.preschedule = T,mc.silent = T))
sum(re.alt.BIC - re.null.BIC < delta.cut["AGEyr"])  #140
#.05*nrow(norm.prt) #123
