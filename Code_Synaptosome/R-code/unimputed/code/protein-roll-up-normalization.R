...
load("../WorkData/QC-SYNAPTOSOME-PEPTIDE.RData")

### select the samples and complete peptides
qc.pep=qc.pep[,samples]
meta.samples=meta.samples[samples,]  # 56

qc.pep=qc.pep[rowSums(is.na(qc.pep)) == 0,]
meta.pep=meta.pep[rownames(qc.pep),]  # 10,761

### selet the peptides linked to a single protein
remove.pep=rownames(meta.pep)[grep("; ",meta.pep$ROLLUP)]   # 877
meta.pep=meta.pep[!(rownames(meta.pep) %in% remove.pep),]   # 9884
