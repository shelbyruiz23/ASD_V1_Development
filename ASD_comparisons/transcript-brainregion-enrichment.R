...
# DE (q/FDR < 0.1) protein and transcripts (Gandal 2022)
...
# (1) Homogenate - 12 lists
# (2) Synaptosome - 24 lists

# load workdata of cell lists
load("ASD_enrichment/WorkData/transcript-regions-list.RData")
########
########
########
######################################
...
# geneclasses are all brain region DE transcripts (n=11 regions)
#geneclasses <- results_Hom.up
#geneclasses <- results_Hom.down
#geneclasses <- results_Syn.up
#geneclasses <- results_Syn.down
#geneclasses <- results_Syn.int.up
geneclasses <- results_Syn.int.down
...
#proteinclasses <- DE_Syn.int.up
proteinclasses <- DE_Syn.int.down
...
#result_dir="ASD_enrichment/Results/transcript_Gandal_hom_up" # Hom_up
#result_dir="ASD_enrichment/Results/transcript_Gandal_hom_down" # Hom_down

#result_dir="ASD_enrichment/Results/transcript_Gandal_syn_up" # Syn_up
#result_dir="ASD_enrichment/Results/transcript_Gandal_syn_down" # Syn_down

#result_dir="ASD_enrichment/Results/transcript_Gandal_syn_int_up" # Syn_int_up
result_dir="ASD_enrichment/Results/transcript_Gandal_syn_int_down" # Syn_int_down
...
#ORmat <- matrix(nrow = length(geneclasses), ncol = 1, dimnames = list(names(geneclasses), as.character("Syn_int_up")))
#logPmat <- matrix(nrow = length(geneclasses), ncol = 1, dimnames = list(names(geneclasses), as.character("Syn_int_up")))
#Pmat <- matrix(nrow = length(geneclasses), ncol = 1, dimnames = list(names(geneclasses), as.character("Syn_int_up")))

ORmat <- matrix(nrow = length(geneclasses), ncol = 1, dimnames = list(names(geneclasses), as.character("Syn_int_down")))
logPmat <- matrix(nrow = length(geneclasses), ncol = 1, dimnames = list(names(geneclasses), as.character("Syn_int_down")))
Pmat <- matrix(nrow = length(geneclasses), ncol = 1, dimnames = list(names(geneclasses), as.character("Syn_int_down")))
#####
############
...
for (i in seq_along(geneclasses)) {
  ...
  #backgroundTotal = background_Syn.int.up[[i]]
  backgroundTotal = background_Syn.int.down[[i]]
  ...
  # Write overlapping genes to a file
  #write(overlap_res[[1]]$overlapping_genes,
   #    file = file.path(result_dir, sprintf("overlap_%s_%s.txt", names(geneclasses)[i],"Syn_int_up")))
  
  # Write overlapping genes to a file
  write(overlap_res[[1]]$overlapping_genes,
       file = file.path(result_dir, sprintf("overlap_%s_%s.txt", names(geneclasses)[i],"Syn_int_down")))
...
# hom
#subfolders=list.files(path = main_directory, pattern = "^transcript_Gandal_hom_", full.names = TRUE)

# syn
subfolders=list.files(path = main_directory, pattern = "^transcript_Gandal_syn_", full.names = TRUE)
...
#write.csv(combined_df, file="ASD_enrichment/Results/overlap/transcript-Gandal-overlap-list_hom.csv")
#write.csv(combined_df, file="ASD_enrichment/Results/overlap/transcript-Gandal-overlap-list_syn.csv")
